...
export default function ProductStatsBar({ products = [] }) {
  const stats = {
    total: products.length,
    active: products.filter(p => p.status === "active").length,
    inactive: products.filter(p => p.status !== "active").length,
    outOfStock: products.filter(p => p.attributes.reduce((s, a) => s + a.stock, 0) === 0).length,
    lowStock: products.filter(p => {
      const t = p.attributes.reduce((s, a) => s + a.stock, 0);
      return t > 0 && t <= 10;
    }).length
  };
