// src/pages/admin/components/customers/CustomerFilters.jsx

import AdminIcon from "../AdminIcons";

export default function CustomerFilters({ filters, onChange, onReset, total }) {
  const handle = key => e => onChange({ ...filters, [key]: e.target.value });
  const hasActiveFilter = filters.search || filters.status || filters.loginMethod;
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <span className="text-xs text-stone-400 ml-auto">
        <span className="font-semibold text-stone-700">{total}</span> khách hàng
      </span>
    </div>
  );
}
